...
namespace Tests.EditMode.GridContents.Units
{
    ...
    public partial class PerformanceTests
    {
        ...
        public void UnitModel_StatusEffectApplication_Performance_IsAcceptable()
        {
            // Arrange
            var stopwatch = new Stopwatch();
            var statusEffect = new MockStatusEffect(StatusEffectType.NotImp);
            ...
            // Cleanup - уничтожаем созданный ScriptableObject если он был создан в тесте
            if (statusEffect != null)
            {
                // Получаем доступ к приватному полю через reflection для cleanup
                var dataField = typeof(StatusEffect).GetField("_data", 
                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                if (dataField?.GetValue(statusEffect) is StatusEffectData data)
                {
                    // Проверяем, был ли это объект, созданный в тесте (по имени)
                    if (data.name.StartsWith("Test "))
                    {
        ...
        public void UnitModel_EventSubscription_Performance_IsAcceptable()
        {
            // Arrange
            var stopwatch = new Stopwatch();
            var eventHandlers = new List<Action<DamageContext>>();

            // Act
            stopwatch.Start();
            for (int i = 0; i < PerformanceTestIterations; i++)
            {
                var handler = new Action<DamageContext>((var ) => { });
