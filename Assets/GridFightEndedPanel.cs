...
public class GridFightEndedPanel : MonoBehaviour, IDisposable
{
    ...
    private IGameplayEventBus _gameplayEvents;
    private readonly CompositeDisposable _disposables = new CompositeDisposable();
    private bool _resultHandled;

    [Inject]
    public void Construct(ITurnStateViewModel turnState, IGameplayEventBus gameplayEvents)
    {
    ...
    private void HandleBattleState(BattleState state)
    {
        ...
        AdventureStateCache.CompleteBattle(playerWon);
        _gameplayEvents?.PublishBattleCompleted(playerWon);
    ...
    {
        var targetScene = AdventureStateCache.GetReturnSceneOrDefault();
